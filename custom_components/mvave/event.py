...
class MvavePadEvent(_MvaveHoldableEvent):
    ...
    def __init__(
        ...
        # The number printed on the pad, which is the device's own: PAD1 is the bottom-left
        # and PAD16 the top-right, so they run up the grid rather than down it.
        #
        # Briefly renamed to reading order on 2026-09-13, on the belief that the device's
        # numbering was a protocol detail nobody could see. It is printed on the hardware —
        # checked by looking, the same afternoon — so that rename moved the one number a
        # person can verify away from the one thing that cannot drift, and made "Pad 1" the
        # top-left pad in Home Assistant while the pad under it said 1 in the bottom-left
        # corner. The configuration screen and `mvave.press_slot` were moved onto the
        # printed numbers to match, rather than this being moved away from them.
        #
